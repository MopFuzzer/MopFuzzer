...
public class RangeChecker extends MutateTemplate_B {
	...
	private class ArrayRangeChecker {
		protected void generatePatches(ITree suspCodeTree) {
			...
			if (conditionalExp.isEmpty()) return;
			if (Checker.isReturnStatement(suspCodeTree.getType())) {
				...
				if (varName != null) {
					codeEndPos = ContextReader.identifyRelatedStatements(suspCodeTree, varName, codeEndPos).getFirst();
				}
	...
	private class CollectionRangeChecker {
		protected void generatePatches(ITree suspStmtTree) {
			...
			if (Checker.isReturnStatement(suspStmtTree.getType())) {
				...
				if (varName != null) {
					codeEndPos = ContextReader.identifyRelatedStatements(suspStmtTree, varName, codeEndPos).getFirst();
				}
			}
