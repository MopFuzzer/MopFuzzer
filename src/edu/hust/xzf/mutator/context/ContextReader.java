...
//            }
    ...
    public static Pair<Integer, Boolean> identifyRelatedStatements(ITree suspStmtTree, String varName, int endPosition) {
        List<String> varNames = new ArrayList<>();
        boolean hasReturn = false;
        varNames.add(varName);
        List<ITree> peerStmts = suspStmtTree.getParent().getChildren();
        boolean isFollowingPeerStmt = false;
        for (int i = 0; i < peerStmts.size(); i++) {
            ITree peerStmt = peerStmts.get(i);
            if (isFollowingPeerStmt) {
                if (Checker.isCatchClause(peerStmt.getType())) break;
                ...
                if (isRelatedStmt) {
                    // for the do while statement, we should not include the last statement.
                    if (!(i == peerStmts.size() - 1 && Checker.isDoStatement(suspStmtTree.getParent().getType()))) {
                        endPosition = peerStmt.getPos() + peerStmt.getLength();
                        if (Checker.isReturnStatement(peerStmtType)) hasReturn = true;
                    }
                }
            } else {
        ...
        return new Pair<>(endPosition, hasReturn);
    }
        ...
        for (ITree child : children) {
            ...
            if (Checker.isSimpleName(childType)) {
                if (child.getLabel().startsWith("MethodName:")) {
                    ...
                    || Checker.isComplexExpression(childType)) {
                if (containsVar(child, varNames))
                    return true;
            }
        }
