...
//                }
                case 1 -> {
                    ...
                    if (Checker.isArrayAccess(var.getType())) {
                        ...
                        fixedCodeStr1 += varType + " " + newVar + " = " + expStr + "\n";
                    offset += 1;
                }
            }
...
//                }
                case 1 -> {
                    ...
                    if (Checker.isVariableDeclarationStatement(stmtAst.getType())) {
                        ITree var = stmtAst.getChild(1).getChild(0);
                        fixedCodeStr1 += "\n" + this.getSubSuspiciouCodeStr(suspCodeStartPos, var.getPos());
                        String newVarStr = this.getSubSuspiciouCodeStr(var.getPos(), var.getEndPos())
                                + generateUniqueVarName();
                        fixedCodeStr1 += newVarStr;
                        fixedCodeStr1 += this.getSubSuspiciouCodeStr(var.getEndPos(), suspCodeEndPos) + "\n";
                        offset += 2;
                    }
                }
            }
